describe("Game", function() {
  ...
  describe("combine", function() {
    it("should for one ingredient", function() {
      ...
      expect(game.combine(Recipe.all.oak_plank)).toBeTruthy();
    ...
    it("should for two ingredients", function() {
      ...
      // by default it should use up make only 1
      expect(game.combine(Recipe.all.wooden_sword)).toBeTruthy();
      expect(game.inventory.find('Oak Plank')).toBeFalsy();
    ...
    it("should not work if not enough ingredients", function() {
      ...
      expect(game.combine(Recipe.all.oak_plank)).toBeFalsy();
    ...
    it("for a higher amount", function() {
      game.inventory.add(Ingredient.oak_wood(4));
      expect(game.inventory.find('Oak Plank')).toBeFalsy();
      expect(game.combine(Recipe.all.oak_plank, 3)).toBeFalsy();
      expect(game.combine(Recipe.all.oak_plank, 2)).toBeTruthy();
      
    });
  });
  describe("findRecipeFor", function() {
    beforeEach(function() {
      game.player.recipes = [Recipe.all.oak_plank, Recipe.all.wooden_sword];
    });
    it("should match", function() {
      expect(game.findRecipeFor([Ingredient.oak_wood(2)])).toEqual(Recipe.all.oak_plank);
    });
    it("should not match", function() {
      expect(game.findRecipeFor([Ingredient.oak_wood(1)])).toBeFalsy();
    });
    it("should match if too many ingredients", function() {
      expect(game.findRecipeFor([Ingredient.oak_wood(3)])).toEqual(Recipe.all.oak_plank);
    });
    it("should match if too many ingredients", function() {
      expect(game.findRecipeFor([Ingredient.oak_plank(1), Ingredient.oak_wood(3)])).toEqual(Recipe.all.oak_plank);
    });
  });
